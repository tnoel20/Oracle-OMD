...
DATA_DIR = '/nfs/hpc/share/noelt/data'#'/home/thomas/data'
...
def main():
    ...
    for lab, fn, dat in tqdm(zip(test_labels, test_filenames, test_data)):
        example = make_example(lab, fn, dat, 'test')
        #example['fold'] = 'test'
        examples.append(example)
    ...
    for idx, split in enumerate(splits):
        ...
        known_examples = [e for e in examples if e.split(',')[1] not in unknown_classes]
        unknown_examples = [e for e in examples if e.split(',')[1] in unknown_classes]
...
def make_example(label, filename, data, fold):
    pixels = data.reshape((3,32,32)).transpose((1,2,0))
    filename = str(filename, 'utf-8')
    Image.fromarray(pixels).save(filename)
    class_name = cifar_class(label)
    return '{},{},{},{},{},{}'.format(os.path.join(DATASET_PATH,filename),class_name,
	is_animal(class_name),is_flying(class_name),is_pet(class_name),fold) 
